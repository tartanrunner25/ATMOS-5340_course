...
Copy a shell script to your directory
    ...
    cp ~u0035056/public_html/5340/first.csh .
...
Copy a scripts from the class directory to your own `public_html` class directory
    ...
    cp ~u0035056/public_html/5340/camera.csh .
    chmod +x camera.csh
    ./camera.csh
...
Lots of options out there. This requires a little more work to use on CHPC... For this class we will avoid these for now.


---

Let’s edit the script on your Mac, not on the meteo07 computer.
- **Type `exit` to log out of the Notchpeak and be back on your local mac.**
...
We want to copy `camera_bflat.csh` to your `public_html/5340` directory in your CHPC account. The `scp` command stands for "secure copy." 

    scp camera_bflat.csh uXXXXXXX@meteo07.chpc.utah.edu:public_html/5340
